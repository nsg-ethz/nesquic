...
//
const QUIC_BUFFER Alpn = { sizeof("perf") - 1, (uint8_t*)"perf" };
    ...
    )
{
    ...
    void* SendBufferRaw = malloc(sizeof(QUIC_BUFFER) + size);
    ...
    //
    QUIC_STATUS Status;
    int flag = (fin) ? QUIC_SEND_FLAG_FIN : 0;
    ...
    )
{
    ...
    case QUIC_STREAM_EVENT_RECEIVE:
        ...
        void* dest = malloc(Event->RECEIVE.Buffers[0].Length);
        ...
        for (uint32_t i = 0; Offset < sizeof(uint64_t) && i < Event->RECEIVE.BufferCount; ++i) {
            ...
            memcpy(dest, Event->RECEIVE.Buffers[i].Buffer, Length);
            Offset += Length;
        }

        DEBUG_PRINT(("Received Data: -%s-\n", (char*)dest));
        ServerSend(Stream, 25000000-SendBufferLength, FALSE);
    ...
    case QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN:
        ...
        ServerSend(Stream, SendBufferLength, TRUE);
        break;
...
QUIC_SETTINGS getCustomSettings(){

    QUIC_SETTINGS Settings = {0};

    Settings.MaxBytesPerKey = 274877906944;
    Settings.HandshakeIdleTimeoutMs = 10000;
    Settings.IdleTimeoutMs = 2000; //og 2000 and after 1000, sync with quinn = 1000
    Settings.MtuDiscoverySearchCompleteTimeoutUs = 600000000;
    Settings.TlsClientMaxSendBuffer = 4096;
    Settings.TlsServerMaxSendBuffer = 8192;
    Settings.StreamRecvWindowDefault = 65536;
    Settings.StreamRecvBufferDefault = 4096;
    Settings.ConnFlowControlWindow = 16777216;
    Settings.MaxWorkerQueueDelayUs = 250000;
    Settings.MaxStatelessOperations = 16;
    Settings.InitialWindowPackets = 10;
    Settings.SendIdleTimeoutMs = 1000;
    Settings.InitialRttMs = 333;
    Settings.MaxAckDelayMs = 200; //og 25
    Settings.DisconnectTimeoutMs = 16000;
    Settings.KeepAliveIntervalMs = 0;
    //QUIC_CONGESTION_CONTROL_ALGORITHM_BBR or QUIC_CONGESTION_CONTROL_ALGORITHM_MAX are also options (bbr requires beta feature flag, see msquic.h)
    Settings.CongestionControlAlgorithm = QUIC_CONGESTION_CONTROL_ALGORITHM_CUBIC; // QUIC_CONGESTION_CONTROL_ALGORITHM
    //Settings.CongestionControlAlgorithm = QUIC_CONGESTION_CONTROL_ALGORITHM_BBR; // QUIC_CONGESTION_CONTROL_ALGORITHM
    Settings.PeerBidiStreamCount = 1;
    Settings.PeerUnidiStreamCount = 0;
    Settings.MaxBindingStatelessOperations = 100;
    Settings.StatelessOperationExpirationMs = 100;
    Settings.MinimumMtu = 1248;
    Settings.MaximumMtu = 1500;
    Settings.SendBufferingEnabled             = 1;
    Settings.PacingEnabled                    = 1;
    Settings.MigrationEnabled                 = 1;
    Settings.DatagramReceiveEnabled           = 0;
    Settings.ServerResumptionLevel           = QUIC_SERVER_RESUME_AND_ZERORTT;    // QUIC_SERVER_RESUMPTION_LEVEL
    Settings.GreaseQuicBitEnabled            = 0;
    Settings.EcnEnabled                      = 0;
    Settings.MaxOperationsPerDrain = 16;
    Settings.MtuDiscoveryMissingProbeCount = 3;
    Settings.DestCidUpdateIdleTimeoutMs = 20000;
    /* Configuration fails with these 3
    Settings.StreamRecvWindowBidiLocalDefault = 0;
    Settings.StreamRecvWindowBidiRemoteDefault = 0;
    Settings.StreamRecvWindowUnidiDefault = 0;
    */
    ...
    Settings.IsSet.CongestionControlAlgorithm = TRUE; // QUIC_CONGESTION_CONTROL_ALGORITHM
    ...
    Settings.IsSet.ServerResumptionLevel           = TRUE;    // QUIC_SERVER_RESUMPTION_LEVEL
